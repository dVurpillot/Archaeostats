...
greppo <- read.csv("D:/Travail/Tableaux/Statistiques/R/bulliot_analyse2.csv", sep=";")
summary(greppo)
res.mca = MCA(greppo, quali.sup=c(24:25), graph=FALSE)
...
res$eig[1:10,]
burt=t(tab.disjonctif(greppo[,1:23]))%*%tab.disjonctif(greppo[,1:25])
...
res.aux = MCA(greppo.comp, quali.sup=c(24:25,26,27,28,29), graph=F)
x11()
plot(res.aux,invisible=c("quali.sup","var","quanti.sup"),habillage=29)
